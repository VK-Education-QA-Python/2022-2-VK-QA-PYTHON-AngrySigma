...
class TestMysql(MyTest):
    ...
    def test_popular_requests(self):
        rows_to_table = 10
        for request in popular_requests(rows_to_table).items():
    ...
    def test_request_methods(self):
        for method in request_methods().items():
    ...
    def test_users_caused_500(self):
        rows_to_table = 5
        for ip in users_caused_500(rows_to_table).items():
    ...
    def test_requests_total(self):
        self.client.create_requests_total(count_requests())
        assert self.get_total_requests()[0].num == 225133

    def test_requests_caused_400(self):
        rows_to_table = 5
        for request in requests_caused_400(rows_to_table).items():
